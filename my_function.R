...
for (index1 in M){
  ...
  g <- ggplot(times, aes(x=Var2, y=Time, color = Var1))+
    geom_line()+
    geom_point()+
    xlab("p")+
    ylab("Log(Time)")+
    ...
    # labs(colour= "Methods", title = paste0("Model (M", index1,")"))
    labs(colour= "Methods", title = paste0("Model (M", 4,")"))
  g
}
...
tab <- rbind(paste0(round(result1,2), '(', round(std1,2), ')'), paste0(round(result2,2), '(', round(std2,2), ')'),
      paste0(round(result3,2), '(', round(std3,2), ')'))

# colnames(tab) <- c('SSDR-SIR', 'Lasso-SIR', 'Lasso', 'Rifle-SIR')
colnames(tab) <- c('SSDR-SIR', 'SSDR-Intra', 'SSDR-PFC', 'Lasso-SIR')
xtable(tab, 
       caption = 'The average distance $\\calD(\\hatbolbeta, \\hatbolbeta^b)$, average rank $\\widehat{d}$, average sparsity $s$ and the corresponding standard error ($\\times 10^{-2}$) based on original samples and 100 bootstrap samples.',
