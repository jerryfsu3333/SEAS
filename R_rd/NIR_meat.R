...
# for(i in seq_len(times)){
  ...
  # fit_intra <- ssdr.cv(train_x, train_y, lam1_fac = seq(2,0.2, length.out = 10), lam2_fac = seq(0.001,0.2, length.out = 10), categorical=TRUE, type = 'intra')
  # d_intra <- fit_intra$rank
  # directions_intra <- svd(fit_intra$Beta)$u[,1:d_intra, drop=FALSE]
  # 
  # new_train <- data.frame(train_x %*% directions_intra)
  # new_test <- data.frame(test_x %*% directions_intra)
  # colnames(new_train) <- paste0('X', seq_len(ncol(new_train)))
  # colnames(new_test) <- paste0('X', seq_len(ncol(new_test)))
  # train_dat <- cbind(y = train_y, new_train)
  # err_intra <- err_func(train_dat, new_test, test_y)

  
  # fit_pfc <- ssdr.cv(train_x, train_y, lam1_fac = seq(1.5,0.2, length.out = 10), lam2_fac = seq(0.001,0.2, length.out = 10), categorical=TRUE,type = 'pfc', cut_y = FALSE, maxit_outer = 1e+4)
  # d_pfc <- fit_pfc$rank
  # directions_pfc <- svd(fit_pfc$Beta)$u[,1:d_pfc, drop=FALSE]
  # new_train <- data.frame(train_x %*% directions_pfc)
  # new_test <- data.frame(test_x %*% directions_pfc)
  # colnames(new_train) <- paste0('X', seq_len(ncol(new_train)))
  # colnames(new_test) <- paste0('X', seq_len(ncol(new_test)))
  # train_dat <- cbind(y = train_y, new_train)
  # err_pfc <- err_func(train_dat, new_test, test_y)
...
err_sir <- do.call(rbind, lapply(err_list, '[[', 1))
err_lassosir <- do.call(rbind, lapply(err_list, '[[', 2))
err_lasso <- do.call(rbind, lapply(err_list, '[[', 3))
err_rifle <- do.call(rbind, lapply(err_list, '[[', 4))
...
#   }
#   
#   output <- list(rank = c(d_sir, d_lassosir, 1, 1), s = c(s_sir, s_lassosir, s_lasso, s_rifle),
