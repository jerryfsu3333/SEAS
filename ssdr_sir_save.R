...
prep <- function(x, y){
  ...
  for (i in 1:nclass){
    tmp <- sigma - cov(x[y == i, ])
    lam_list <- rbind(lam_list, svd(tmp)$d)
    mu <- cbind(mu, tmp)
  }
  output <- list(sigma = sigma, mu = mu, lam_list = lam_list)
  return(output)
}
...
# set.seed(1)
# 
# p <- 50  # Dimension of observations
# N <- 1000  # Sample size
...
for(t in 1:times){
  ...
  # output <- prep(x_train, y_train)
  # lam_list <- output$lam_list
  ################################################
  # MSDA
  ################################################
