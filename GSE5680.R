...
x <- scan('~/Documents/GitHub/ssdr/Real_dataset/GSE5680_x.txt')
y <- scan('~/Documents/GitHub/ssdr/Real_dataset/GSE5680_y.txt')
x <- matrix(x, nrow = 117)
...
#   })
#   ord <- order(dist_cor, decreasing = TRUE)[1:2000]
# 
...
#   test_x <- test_x[,ord]
# 
#   fit_sir <- ssdr.cv(train_x, train_y, nfolds = 3, lam1_fac = seq(2,0.8, length.out = 10), lam2_fac = seq(0.001,0.2, length.out = 10), type = 'sir')
...
# 
# 
#   fit_intra <- ssdr.cv(train_x, train_y, nfolds = 3, lam1_fac = seq(2,0.8, length.out = 10), lam2_fac = seq(0.001,0.2, length.out = 10), type = 'intra')
...
#   fit_pfc <- ssdr.cv(train_x, train_y, nfolds = 3, lam1_fac = seq(1.5,1.1, length.out = 10), lam2_fac = seq(0.001,0.2, length.out = 10), type = 'pfc', cut_y = TRUE, maxit_outer = 1e+4)
...
output_func <- function(x, y){
  ...
  fit_sir <- ssdr.cv(x, y, lam1_fac = seq(2,0.8, length.out = 10), lam2_fac = seq(0.001,0.2, length.out = 10), nfolds = 3, type = 'sir')
  if(!is.numeric(fit_sir$Beta)){
  ...
  fit_intra <- ssdr.cv(x, y, lam1_fac = seq(2,0.8, length.out = 10), lam2_fac = seq(0.001,0.2, length.out = 10), nfolds = 3, type = 'intra')
  if(!is.numeric(fit_intra$Beta)){
    warning('A NULL matrix is returned (intra).')
    d_intra <- NA
    directions_intra <- NA
  ...
  fit_pfc <- ssdr.cv(x, y, lam1_fac = seq(1.5,1.1, length.out = 10), lam2_fac = seq(0.001,0.2, length.out = 10), nfolds = 3, type = 'pfc', cut_y = TRUE, maxit_outer = 1e+4)
  if(!is.numeric(fit_pfc$Beta)){
    warning('A NULL matrix is returned (intra).')
    d_pfc <- NA
    directions_pfc <- NA
